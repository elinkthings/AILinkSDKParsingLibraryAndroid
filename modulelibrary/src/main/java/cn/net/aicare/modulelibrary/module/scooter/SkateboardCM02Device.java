...
public class SkateboardCM02Device extends BaseBleDeviceData implements OnBleOtherDataListener {
    private String TAG = SkateboardCM02Device.class.getName();
...
//        byte[] cmd = new byte[]{SkateboardBleConfig.PREFIX, SkateboardBleConfig.HOST_WRITE_RETURN_METER, (byte) 0x0C, (byte) 0x0C, 0x01, (byte) 0x01};
    ...
    public void setUpdateData(String filePath, @SkateboardBleConfig.OTAType int type, @SkateboardBleConfig.ScopeType int scope) {
        try {
            ...
            mScooterOtaManager = new ScooterCM02OtaManager();
    ...
    public synchronized void sendUpdateData(int status, int id) {
        ...
        if (mScooterOtaManager == null) {
            //当前不在升级状态
            BleLog.i("当前不在升级状态status:"+status+"  id:"+id);
            return;
        }
    ...
    private void dataCheck(byte[] data) {
        ...
        if ((cmdStart == 0xE1 && cmdEnd == 0x00) || (cmdStart == 0xF1 && cmdEnd == 0x00) || (cmdStart == 0xD1 && cmdEnd == 0x00)) {
            int id = 1;
            if (data.length > 7) {
            ...
            mOtaSendErr = 1;
            return;
        } else if ((cmdStart == 0xE2 && cmdEnd == 0x00) || (cmdStart == 0xF2 && cmdEnd == 0x00) || (cmdStart == 0xD2 && cmdEnd == 0x00)) {
            //升级结束应答
            if (mScooterOtaManager==null){
                //
