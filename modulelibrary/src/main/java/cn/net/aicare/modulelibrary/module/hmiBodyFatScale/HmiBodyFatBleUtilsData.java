package cn.net.aicare.modulelibrary.module.hmiBodyFatScale;
...
public class HmiBodyFatBleUtilsData extends BaseBleDeviceData {
    ...
    public void onNotifyData(String uuid, byte[] hex, int type) {
        try {
            ...
            dataAnalysis(hex, cmd);
        } catch (Exception e) {
            BleLog.e("HMI体脂秤,数据解析异常:" + BleStrUtils.byte2HexStr(hex));
        }
    }
     ...
     * @param cmd
     */
    private void dataAnalysis(byte[] hex, int cmd) {
        switch (cmd) {
            //实时体重
            ...
            case HmiBodyFatDataUtil.WEIGHT_RESULT:
                if (mBleBodyFatCallback != null) {
                    mBleBodyFatCallback.onWeightData(cmd, HmiBodyFatDataUtil.getInstance().getWeight(hex), HmiBodyFatDataUtil.getInstance().getWeightUnit(hex), HmiBodyFatDataUtil.getInstance().getWeightPoint(hex));
                }
                break;
